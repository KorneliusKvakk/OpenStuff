...
class Genome:
    ...
    def AddNodeGene(self, newNodeGene : NodeGene):
        self.NodeGenes[newNodeGene.GetNodeId()] = newNodeGene

    def AddConnectionGene(self, newConnectionGene : ConnectionGene):
        self.ConnectionGenes[newConnectionGene.GetInnovation()] = newConnectionGene

    def SetConnectionWeight(self, inNode : int, outNode : int, newWeight : float):
        for connectionGeneId, connectionGene in self.ConnectionGenes.items():
    ...
    def MutateAddNode(self):
        ...
        inNodeGene = self.NodeGenes[suitableConnectionGene.GetInNode()]
        outNodeGene = self.NodeGenes[suitableConnectionGene.GetOutNode()]
    ...
    def MutateAddConnection(self):
        ...
        randomNodeGeneIn = random.choice(list(self.NodeGenes.values()))
        randomNodeGeneOut = random.choice(list(self.NodeGenes.values()))
        # return if connection is impossible
        if randomNodeGeneIn.networkNodeType == NetworkNodeType.OUTPUT:
            return
        if randomNodeGeneIn.networkNodeType == NetworkNodeType.INPUT and randomNodeGeneOut.networkNodeType == NetworkNodeType.INPUT:
            return
    ...
    def MutateConnectionWeight(self):
        ...
        if roll < perturbingProbability:
            ...
            self.ConnectionGenes[enabledConnectionGene.GetInnovation()].SetWeight(newWeight)
        ...
        while len(nextConnectionGenes) > 0:
            for nextConnectionGene in nextConnectionGenes:
                if nextConnectionGene.IsEnabled() == True and nextConnectionGene.IsRecurrent() == False:
                    ...
                    for outputConnectionGeneId, outputConnectionGene in self.ConnectionGenes.items():
