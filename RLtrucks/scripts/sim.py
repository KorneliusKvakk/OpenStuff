...
class Sim:
    def __init__(self, draw_env):
        ...
        self.n_packages_delivered_in_step = 0
         
        # initialize depots resultant angle for rule based control
        self.depots_resultant_angle_deg = 0
    ...
    def run_sim(self, action_deg):
        ...
        # fix floating point error in trucks with full capacity (empty trucks)
        for truck in self.trucks:
            if truck.capacity_kg > truck.capacity_kg_0 - 1:
                        ...
                        if not truck.ready_to_go:
                            ...
                            if depot.name not in loaded_packages_destination_name:
                                ...
                                if self.timestep_sim % off_load_time == 0:
                                    ...
                                    self.n_packages_delivered_in_step += 1
        ...
        if update_action:
            for truck in self.trucks:
                # truck postition
                truck_position_x = truck.position[0]
                truck_position_y = truck.position[1]
                # get truck capacity
                ...
                depots_vector = []
                depots_n_packages = []
                depots_weight_kg_packages = []
                for depot in self.depots:
                    n_packages_for_depot = 0
                    weight_kg_packages_for_depot = 0
                    for package in truck.loaded_packages:
                        if package.destination_name == depot.name:
                            n_packages_for_depot += 1
                            weight_kg_packages_for_depot += package.weight_kg
                    if n_packages_for_depot > 0:
                        delta_x = n_packages_for_depot * (depot.position_x - truck_position_x)
                        delta_y = n_packages_for_depot * (depot.position_y - truck_position_y)
                        depot_vector = (delta_x, delta_y)
                        depots_vector.append(depot_vector)
                    depots_n_packages.append(n_packages_for_depot)
                    depots_weight_kg_packages.append(weight_kg_packages_for_depot)
                # if truck is empty, add vector pointing to hub
                if truck.capacity_kg == truck.capacity_kg_0:
                    delta_x = (self.hubs[0].position_x - truck_position_x)
                    delta_y = (self.hubs[0].position_y - truck_position_y)
                    depot_vector = (delta_x, delta_y)
                    depots_vector.append(depot_vector)
                
                _depots_resultant_vector, depots_resultant_magnitude, depots_resultant_angle_deg = calculate_resultant_vector(depots_vector)

            # update depots resultant angle for rule based control
            self.depots_resultant_angle_deg = depots_resultant_angle_deg

            obs = [truck_capacity_kg, self.truck00_fuel_consumption[-1], truck_position_int, len(available_roads),
                   depots_n_packages[0], depots_weight_kg_packages[0],
                   depots_n_packages[1], depots_weight_kg_packages[1],
                   depots_n_packages[2], depots_weight_kg_packages[2],
                   depots_n_packages[3], depots_weight_kg_packages[3],
                   depots_n_packages[4], depots_weight_kg_packages[4],
                   depots_n_packages[5], depots_weight_kg_packages[5]]

                #    depots_resultant_angle_deg, depots_resultant_magnitude]

        reward = -1.5 + self.n_packages_delivered_in_step
        ...
        # reward = -1 + self.n_packages_delivered_in_step

        # reward = -1.2 + reward_packages_delivered * 2 + reward_km_driven + reward_fuel_consumption
